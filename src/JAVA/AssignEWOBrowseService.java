...
public class AssignEWOBrowseService {
    ...
    public String getAssignEWOBrowseToJSON(MySqlDbConnection db) {
        ...
        try {
            ...
            getAvailability(id_maint, h8to9, h9to10, h10to11, h11to12, h14to15, h15to16, h16to17, "Monday");
    ...
    private void getAvailability(ArrayList<Integer> id_maint, ArrayList<Integer> h8to9, ArrayList<Integer> h9to10, ArrayList<Integer> h10to11, ArrayList<Integer> h11to12, ArrayList<Integer> h14to15, ArrayList<Integer> h15to16, ArrayList<Integer> h16to17, String day) {
        try {
            ...
            for (int i = 0; i < id_maint.size(); i++) {
                ...
                rs = stmt.executeQuery("SELECT avail_8to9 FROM availability WHERE id_maint=" + id_maint.get(i) + " AND day=\"" + day + "\"");
                while (rs.next()) {
                    h8to9.set(i, rs.getInt(1));
                }

                //conteggio availability manutentori 9to10
                rs = stmt.executeQuery("SELECT avail_9to10 FROM availability WHERE id_maint=" + id_maint.get(i) + " AND day=\"" + day + "\"");
                while (rs.next()) {
                    h9to10.set(i, rs.getInt(1));
                }
                //conteggio availability manutentori 10to11
                rs = stmt.executeQuery("SELECT avail_10to11 FROM availability WHERE id_maint=" + id_maint.get(i) + " AND day=\"" + day + "\"");
                while (rs.next()) {
                    h10to11.set(i, rs.getInt(1));
                }

                //conteggio availability manutentori 11to12
                rs = stmt.executeQuery("SELECT avail_11to12 FROM availability WHERE id_maint=" + id_maint.get(i) + " AND day=\"" + day + "\"");
                while (rs.next()) {
                    h11to12.set(i, rs.getInt(1));
                }

                //conteggio availability manutentori 14to15
                rs = stmt.executeQuery("SELECT avail_14to15 FROM availability WHERE id_maint=" + id_maint.get(i) + " AND day=\"" + day + "\"");
                while (rs.next()) {
                    h14to15.set(i, rs.getInt(1));
                }

                //conteggio availability manutentori 15to16
                rs = stmt.executeQuery("SELECT avail_15to16 FROM availability WHERE id_maint=" + id_maint.get(i) + " AND day=\"" + day + "\"");
                while (rs.next()) {
                    h15to16.set(i, rs.getInt(1));
                }

                //conteggio availability manutentori 16to17
                rs = stmt.executeQuery("SELECT avail_16to17 FROM availability WHERE id_maint=" + id_maint.get(i) + " AND day=\"" + day + "\"");
                while (rs.next()) {
