...
public class ShowInProgressEWOBrowseService {
    int id_EWO;
    String area;
    String type;
    int estim_time;
    ...
    public String getShowInProgressEWOBrowseToJSON(MySqlDbConnection db) {
        ...
        try {
            ...
            rs = stmt.executeQuery("SELECT specifications, stateDep, stateMaint, stateSpecif FROM InProgress WHERE specifications IN (SELECT s.id FROM specifications as s, activity as a WHERE s.id_activity = a.id AND s.week_activity=a.week AND a.ewo=true AND a.week=" + 1 + ")");
            //JSON realization
            while (rs.next()) {
                removeComma = true;
                //fetch specific EWO
                this.getEWO(rs.getInt(1), 1);
